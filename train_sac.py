...
def train_sac(env_name : str, n_episodes : int, seed : int, batch_size : int, demonstrate : bool):
    ...
    for episode in range(n_episodes):
        ...
        while not done:
            ...
            sac_agent.remember(observation, action, reward, new_observation, done, bc = 0, imagined = 0)
            state_batch, action_batch, reward_batch, new_state_batch, done_batch, bc_batch, imagined = sac_agent.get_batch()
        ...
        # Demonstrate
        if demonstrate and (episode + 1) % HyperParams.demonstrate_every == 0:
            demonstration = demonstrate(agent = sac_agent, env = demo_env)
            demo_idx = len(os.listdir(demo_dir)) + 1
            demo_path = f"{demo_dir}/demo_{demo_idx:04d}.json"
            with open(demo_path, 'w') as outfile:
                json.dump(demonstration, outfile, indent = 4)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--demonstrate', type = bool, default = False, help = 'Generate demonstrations')
